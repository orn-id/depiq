...
By default when interpolating `time.Time` (and `*time.Time`) `depiq` will convert it `UTC` before interpolating.
...
`depiq` provides a **_global_** configuration settings to set the [location](https://golang.org/pkg/time/#Location) to convert all timestamps to. 

To change the default timezone to covert time instances to you can use [`depiq.SetTimeLocation`](https://godoc.org/github.com/orn-id/depiq#SetTimeLocation) to change the default timezone.
