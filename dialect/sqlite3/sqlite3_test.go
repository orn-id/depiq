...
import (
	...
	"github.com/orn-id/depiq/v9/dialect/sqlite3"
...
func (st *sqlite3Suite) TestCompoundQueries() {
	ds1 := st.db.From("entry").Select("int").Where(depiq.C("int").Gt(0))
	ds2 := st.db.From("entry").Select("int").Where(depiq.C("int").Gt(5))
