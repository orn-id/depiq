...
func (ssgs *selectSQLGeneratorSuite) TestToSelectSQL_withFor() {
	...
	scFnSlOf := sc.SetLock(exp.NewLock(exp.ForNolock, exp.SkipLocked, depiq.T("my_table")))
	...
	scFsSlOf := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, depiq.T("my_table")))
	scFsSlOfMulti := sc.SetLock(exp.NewLock(exp.ForShare, exp.SkipLocked, depiq.T("my_table"), depiq.T("table2")))
