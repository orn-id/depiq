...
By default the `depiq` will interpolate all parameters, if you do not want to have values interpolated you can use the [`Prepared`](http://godoc.org/github.com/orn-id/depiq#SelectDataset.Prepared) method to prevent this.
...
sql, args, _ := preparedDs.Where(depiq.Ex{
	"col1": "a",
	"col2": 1,
	"col3": true,
	"col4": false,
...
```go
var items []Item
sql, args, _ := db.From("items").Prepared(true).Where(depiq.Ex{
	"col1": "a",
	"col2": 1,
}).ScanStructs(&items)
