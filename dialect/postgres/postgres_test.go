...
func (pt *postgresTest) TestInsertReturning() {
	...
	now := time.Now()
	e := entry{Int: 10, Float: 1.000000, String: "1.000000", Time: now, Bool: true, Bytes: []byte("1.000000")}
	found, err := ds.Insert().Rows(e).Returning(depiq.Star()).Executor().ScanStruct(&e)
	pt.NoError(err)
	pt.True(found)
	pt.True(e.ID > 0)
...
func (pt *postgresTest) TestUpdate() {
	...
	var id uint32
	found, err = ds.Where(depiq.C("int").Eq(11)).
		Update().
...
func (pt *postgresTest) TestInsert_OnConflict() {
	...
	_, err = ds.Insert().
		Rows(entries).
		OnConflict(depiq.DoUpdate("int", depiq.Record{"string": "upsert"}).Where(depiq.I("excluded.int").Eq(2))).
		Executor().
		Exec()
	pt.NoError(err)
